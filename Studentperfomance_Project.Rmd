...
Plotting a boxplot graph to show comparison in exam score performance according to gender male and female.
...
ggplot(long_data, aes(x = subject, y = score, fill = race.ethnicity)) +
  ...
  labs(title = "Average Scores by Race across Subjects",
       x = "Subject",
       y = "Average Score") +
  theme_minimal()
...
```
Plotting a stacked bar graph for math scores according to lunch type
```{r}
...
server <- function(input, output) {
  ...
  Student_data <- data %>%
    mutate(AverageScore = (MathScore + ReadingScore + WritingScore) / 3)
